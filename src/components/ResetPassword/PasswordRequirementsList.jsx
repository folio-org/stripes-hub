...
import { Layout } from '../../StripesComponents';

import usePasswordRules from './usePasswordRules';

import styles from './PasswordRequirementsList.module.css';
...
const PasswordRequirementsList = ({ passwordValue, config, tenant }) => {
  const intl = useIntl();

  const [requiredRules, setRequiredRules] = useState([]);
  const [unfulfilledRules, setUnfulfilledRules] = useState([]);

  const { rules } = usePasswordRules(rulesLimit, config, tenant, intl.locale);
      ...
      const requiredRulesSet = rules
        ...
        .map(rule => {
          ...
          rule.description = intl.formatMessage(
            { id: 'stripes-hub.createResetPassword.ruleTemplate' },
